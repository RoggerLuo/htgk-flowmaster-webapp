...
function Comp({ inputData, oninput }){
    let value = inputData.value || '' //不加这一句就会报错，因为没有初始值，所以就变成了uncontrolled组件
    let text = inputData.text || '' //不加这一句就会报错，因为没有初始值，所以就变成了uncontrolled组件
    let part1 = text.split('天')
    let part2 = part1[1].split('小时')
    let days = part1[0]
    let hours = part2[0]
    
    // let days = value && parseInt(value/60/60/24) || ''
    ...
    const whenChange1 = (event) => {
        ...
        const newInputData = {
            ...
            value:event.target.value*60*60*24+event.target.parentNode.children[1].value*60*60
        }
    ...
    const whenChange2 = (event) => {
        ...
        const newInputData = {
            ...
            value: event.target.value*60*60 + event.target.parentNode.children[0].value*60*60*24
        }
        console.log(newInputData.value)
        oninput(newInputData)
    }
...
/* <input type='text' value={value} onChange={whenChange}/>分钟 */
export default Comp
