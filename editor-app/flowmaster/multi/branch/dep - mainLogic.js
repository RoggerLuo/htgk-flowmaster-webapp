...
export function naming_multiBranch_usingGateway(selectedShape) { //这个selectedShape是"Exclusive gateway"
    ...
    if (!sf) return true
    if (!sf.properties.conditionsequenceflow) autoNaming(sf, selectedShape) //如果第一根线没有设置过
    //另一根线
    const sf2 = selectedShape.outgoing[1] || false //接着判断第二根线
    if (!sf2) return true
    if (!sf2.properties.conditionsequenceflow) autoNaming(sf2, selectedShape)
}
